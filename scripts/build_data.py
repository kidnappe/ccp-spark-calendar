...
from sync_changelog import write_changelog_json
...
def main():
    ...
    # 更新日志：CHANGELOG.md → changelog.json（单一事实源；页面运行时加载，不再内嵌进 index.html）
    md = open(os.path.join(ROOT, "CHANGELOG.md"), encoding="utf-8").read()
    n_log = write_changelog_json(md)
    print("已重写 index.html 的 historyData / CAUSAL / CAUSAL_INFERRED / CAUSAL_LAYOUT")
    print(f"已生成 changelog.json（更新日志 HTML {n_log} 字符，页面运行时加载）")
